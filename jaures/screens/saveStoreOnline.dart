...
//saveStoreOnline
import 'package:flutter_complete_guide/widgets/saveStoreOnline/save_as.dart';
...
class _TaskManager extends State<TaskManager> {
  ...
  Future<firebase_storage.UploadTask> uploadFile(PickedFile file) async {
    ...
    firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance.ref().child('playground').child('/some-image.jpg');
  ...
  firebase_storage.UploadTask uploadString() {
    ...
    // Create a Reference to the file
    firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance.ref().child('playground').child('/put-string-example.txt');

    // Start upload of putString
    return ref.putString(putStringText,
        metadata: firebase_storage.SettableMetadata(contentLanguage: 'en', customMetadata: <String, String>{
          'example': 'putString'
        }));
  }
    ...
    return StreamBuilder<firebase_storage.TaskSnapshot>(
      ...
      builder: (
        ...
        if (asyncSnapshot.hasError) {
          if (asyncSnapshot.error is firebase_core.FirebaseException && (asyncSnapshot.error as firebase_core.FirebaseException).code == 'canceled') {
