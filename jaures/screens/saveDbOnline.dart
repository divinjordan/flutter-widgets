...
import 'package:flutter_complete_guide/widgets/saveBD/movie.dart';
...
/// are type-safe.
final moviesRef = FirebaseFirestore.instance.collection('firestore-example-app').withConverter<Movie>(
  ...
  Query<Movie> queryBy(MovieQuery query) {
    switch (query) {
      case MovieQuery.fantasy:
        return where('genre', arrayContainsAny: [
          'Fantasy'
        ]);
...
//  late var _likes = Widget.currentLikes;
  var _likes;
  Future<void> _onLike() async {
    ...
    try {
      ...
      // count on the server.
      var newLikes = await FirebaseFirestore.instance.runTransaction<int>((transaction) async {
        ...
        var updatedLikes = movie.data().likes + 1;
