          ...
          children: [
            ...
            if (widget.user != null)
              Column(
                ...
                children: [
                  Text(
                    widget.user.providerData.isEmpty ? 'No providers' : 'Providers:',
                  ...
                  for (var provider in widget.user.providerData)
                    Dismissible(
                      ...
                      child: Card(
                        ...
                        child: ListTile(
                          leading: provider.photoURL == null ? IconButton(icon: const Icon(Icons.remove), onPressed: () => widget.user.unlink(provider.providerId)) : Image.network(provider.photoURL),
                          title: Text(provider.providerId),
                          subtitle: Text("${provider.uid == null ? "" : "ID: ${provider.uid}\n"}"
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        TextButton(
          onPressed: () {
            widget.user.updateProfile(displayName: _nameController.text, photoURL: _urlController.text);
...
class _EmailLinkSignInSectionState extends State<_EmailLinkSignInSection> {
  ...
  Future<void> _signInWithEmailAndLink() async {
    try {
      ...
      await _auth.sendSignInLinkToEmail(email: _userEmail, actionCodeSettings: ActionCodeSettings(url: 'https://react-native-firebase-testing.firebaseapp.com/emailSignin', handleCodeInApp: true, iOSBundleId: 'io.flutter.plugins.firebaseAuthExample', androidPackageName: 'io.flutter.plugins.firebaseauthexample'));
...
class _PhoneSignInSectionState extends State<_PhoneSignInSection> {
  ...
  Future<void> _verifyWebPhoneNumber() async {
    ConfirmationResult confirmationResult = await _auth.signInWithPhoneNumber(_phoneNumberController.text);
  ...
  Future<void> _confirmCodeWeb() async {
    if (webConfirmationResult != null) {
      ...
      widget._scaffold.showSnackBar(const SnackBar(
        content: Text('Please input sms code received after verifying phone number'),
      ));
    }
  }
  ...
  Future<void> _verifyPhoneNumber() async {
    ...
    try {
      await _auth.verifyPhoneNumber(phoneNumber: _phoneNumberController.text, timeout: const Duration(seconds: 5), verificationCompleted: verificationCompleted, verificationFailed: verificationFailed, codeSent: codeSent, codeAutoRetrievalTimeout: codeAutoRetrievalTimeout);
    } catch (e) {
      widget._scaffold.showSnackBar(SnackBar(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Container(
                ...
                alignment: Alignment.center,
                child: kIsWeb
                    ? const Text('When using Flutter Web, API keys are configured through the Firebase Console. The below providers demonstrate how this works')
                    : const Text('We do not provide an API to obtain the token for below providers apart from Google '
              ...
              Container(
                ...
                alignment: Alignment.center,
                child: SignInButton(
                  _provider == 'GitHub' ? Buttons.GitHub : (_provider == 'Facebook' ? Buttons.Facebook : (_provider == 'Twitter' ? Buttons.Twitter : Buttons.GoogleDark)),
                  text: 'Sign In',
                  onPressed: () async {
  ...
  Future<void> _signInWithTwitter() async {
    try {
      ...
      if (kIsWeb) {
        ...
        final AuthCredential credential = TwitterAuthProvider.credential(accessToken: _tokenController.text, secret: _tokenSecretController.text);
        userCredential = await _auth.signInWithCredential(credential);
      }
